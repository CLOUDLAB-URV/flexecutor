...
class TestFlexInput(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.flex_input = FlexInput(
            prefix=self.prefix,
            bucket=self.bucket,
            custom_input_id=self.custom_input_id,
    ...
    def test_scan_objects(self):
        ...
        print(f"Expected keys: {expected_keys}, Actual keys: {self.flex_input.keys}")
        print(
            f"Expected paths: {expected_local_paths}, Actual paths: {self.flex_input.local_paths}"
        )

    def test_broadcast_strategy(self):
        print("Testing scan_objects with BROADCAST strategy")
        self.flex_input.strategy = StrategyEnum.BROADCAST
        self.flex_input.scan_objects(worker_id=0, num_workers=2)
        expected_file_index = (0, 2)
        self.assertEqual(self.flex_input.file_index, expected_file_index)
    ...
    def test_scatter_strategy(self):
        ...
        for worker_id in range(num_workers):
            self.flex_input.scan_objects(worker_id, num_workers)
            start, end = self.flex_input.file_index
            handled_files = self.flex_input.local_paths[start:end]

...
class TestStageContext(unittest.TestCase):
    def setUp(self):
        ...
        self.flex_input = FlexInput(
            prefix="test_prefix/",
            bucket="test_bucket",
            custom_input_id="input123",
            strategy=StrategyEnum.SCATTER,
            local_base_path="/tmp",
        )
        self.flex_output = FlexOutput(
            prefix="test_output/",
            bucket="test_bucket",
            custom_output_id="output123",
            suffix=".file",
            local_base_path="/tmp",
        )
    ...
    def test_initialization(self):
        ...
        self.assertEqual(self.internal_context.outputs["output123"], self.flex_output)
    ...
    def test_next_output_path(self):
        ...
        self.assertIn(
            f"{self.flex_output.prefix}/{os.path.basename(output_path)}",
            self.flex_output.keys,
        )
